...
if action = "show" then
	...
	if showList = 1 then
		conn.execute("update [blog] set show=0 where id = "&id&"")
	else
		conn.execute("update [blog] set show=1 where id = "&id&"")
	end if
	response.Redirect("blogList.asp?title="&title&"")
end if
%>
<!doctype html>
<html>
        ...
        <div class="col-sm-12">
            <div class="panel panel-default">
                <div class="panel-heading clearfix">
                    ...
                    <input type="button" value="新增" class="btn btn-success btn-sm pull-right" style="margin:0 10px" onClick="location='blog.asp?action=add&title=<%=title%>'">
                </div>
                <div class="panel-body">
                    <table class="table table-bordered table-hover">
                        ...
                        sql = "select * from [blog] order by upTime desc,id desc"
                        rs.open sql,conn,1,1
                        if not rs.eof then
                            do while not rs.eof
                            %>
                                <tr>
                                    <td title="<%=rs("blogTitle")%>"><%=rs("blogTitle")&"&nbsp;&nbsp;("&rs("categories")&")"%></td>
                                    <td><%=datemate(rs("upTime"))%></td>
                            ...
                            response.Write "<td colspan='5'>&nbsp;暂无文章</td>"
                        end if
                        rs.close
                        set rs = nothing
                        %>
