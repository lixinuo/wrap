...
if action = "del" then 
	conn.execute("delete from [blogtype] where id = "&id&"")
end if

if action = "show" then
	showList = request("showList")
	if showList = 1 then
		conn.execute("update [blogtype] set show=0 where id = "&id&"")
	else
		conn.execute("update [blogtype] set show=1 where id = "&id&"")
	end if
	response.Redirect("blogtypeList.asp?title="&title&"")
end if
%>
<!doctype html>
<html>
        ...
        <div class="col-sm-12">
            <div class="panel panel-default">
                <div class="panel-heading clearfix">
                    ...
                    <input type="button" value="新增" class="btn btn-success btn-sm pull-right" style="margin:0 10px" onClick="location='blogtype.asp?action=add&title=<%=title%>'">
                </div>
                <div class="panel-body">
                    <table class="table table-bordered table-hover">
                        ...
                        sql = "select * from [blogtype] order by setTime desc,id desc"
                        ...
                        if not rs.eof then
                            ...
                            response.Write "<td colspan='6'>&nbsp;暂无文章类别</td>"
                        end if
                        rs.close
                        set rs = nothing
                        %>
